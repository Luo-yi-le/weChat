...
export class NacosConfiguration {
    ...
    async onReady(container) {
        ...
        const nacosRegistry: INacosRegistry = this.app.getConfig('nacos.registry.client')
        const nacosConfig: INacosConfigClient = this.app.getConfig('nacos.config.client')
        ...
        if (nacosRegistry && nacosRegistry.server && nacosRegistry.server.length) {
            nacosRegistry.server.forEach(item => {
                setNacosRegistry.registerInstance(item.serviceName, item.instance, item.groupName)
            })
        }

        if (nacosConfig && nacosConfig.server && nacosConfig.server.length) {
            nacosConfig.server.forEach(item => {
                setNacosConfig.publishSingle(item.dataId, item.group, item.content, item.options)
            })
        }
