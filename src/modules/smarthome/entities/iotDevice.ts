...
export class IoTDevice extends BaseEntity {
    ...
    @Column({ type: 'tinyint', comment: '设备状态（1-未激活，2-禁用，3-在线，4-离线）', default: 1, nullable: true })
    status: number;


    @Column({ type: 'tinyint', comment: '信号强度（\r\n信号极好4格[-55— 0]，\r\n信号好3格[-70— -55]，\r\n信号一般2格[-85— -70]，\r\n信号差1格[-100— -85]）', nullable: true })
    rssi: number;


    @Column({ type: 'tinyint', comment: '定位方式(1=ip自动定位，2=设备定位，3=自定义)', nullable: true })
    location_way: number;
    ...
    @Column({ comment: '设备摘要，格式[{\"name\":\"device\"},{\"chip\":\"esp8266\"}]', type: 'simple-array' })
    summary: any[];
